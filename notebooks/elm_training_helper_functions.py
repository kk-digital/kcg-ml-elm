...
def get_train_test(
  ...
  #  tag_n_test = int(test_per*len(tag_all_emb_list)) if  int(test_per*len(tag_all_emb_list))  > 0 else 1 
    ...
    train_indices_tag = random.sample(range(len(tag_all_emb_list)), n)
     
  #  test_tag_emb_list    = tag_all_emb_list[: tag_n_test] # test tag/class embeddings. 
    test_indices_tag = [i for i in range(len(tag_all_emb_list)) if i not in train_indices_tag]
    train_tag_emb_list = [tag_all_emb_list[i] for i in train_indices_tag]
   ...
   # other_n_test = int(test_per*len(other_all_emb_list)) if  int(test_per*len(other_all_emb_list))  > 0 else 1 
    ...
    train_indices_others = random.sample(range(len(other_all_emb_list)), n)
     
  #  test_tag_emb_list    = tag_all_emb_list[: tag_n_test] # test tag/class embeddings. 
    test_indices_others = [i for i in range(len(other_all_emb_list)) if i not in train_indices_others]
    train_other_emb_list = [other_all_emb_list[i] for i in train_indices_others]
    test_other_emb_list = [other_all_emb_list[i] for i in test_indices_others]
    other_n_test=len(test_other_emb_list)
